...
/* Global HLE graphics frameskip.
 *  0  = disabled
 * -1  = automatic catch-up
 *
 * Runtime testing showed that occasional automatic HLE skips work well as a
 * renderer-independent fallback, while forced periodic manual skips can break
 * renderer presentation state. Manual modes therefore live in the renderers
 * that know how to keep their own state coherent.
 *
 * A skip decision is cached for all graphics DLists in the current VI slice.
 * LLE RSP tasks are deliberately never discarded. If native gles2n64
 * frameskip is enabled, it takes precedence and this global layer does not
 * skip gles2n64 DLists. */
#define RETRO_FRAMESKIP_AUTO     (-1)
#define RETRO_FRAMESKIP_AUTO_MAX 2u

static int      retro_frameskip_mode = 0;
static unsigned retro_frameskip_auto_skip_pending = 0;
static unsigned retro_frameskip_auto_consecutive = 0;
static uint64_t retro_frameskip_initial_usec = 0;
static uint64_t retro_frameskip_virtual_count = 0;

static uint64_t retro_frameskip_vi_serial = 0;
...
uint64_t parallel_n64_get_time_usec(void)
{
   if (perf_cb.get_time_usec)
      return (uint64_t)perf_cb.get_time_usec();

   return 0;
}

static void retro_frameskip_reset(void)
{
   retro_frameskip_auto_skip_pending = 0;
   retro_frameskip_auto_consecutive = 0;
   retro_frameskip_initial_usec = 0;
   retro_frameskip_virtual_count = 0;
   retro_frameskip_decision_vi = (uint64_t)-1;
   retro_frameskip_skip_current_vi = 0;
}

static void retro_frameskip_set_mode(int mode)
{
   mode = (mode == RETRO_FRAMESKIP_AUTO) ? RETRO_FRAMESKIP_AUTO : 0;

   if (retro_frameskip_mode != mode)
   {
      retro_frameskip_mode = mode;
      retro_frameskip_reset();
   }
}

static void retro_frameskip_begin_vi(void)
{
   retro_frameskip_vi_serial++;
}

/* Called by the HLE RSP before forwarding a graphics DList. */
int retro_frameskip_should_skip_dlist(void)
{
   if (frame_hidden || rsp_plugin != RSP_HLE ||
       retro_frameskip_mode != RETRO_FRAMESKIP_AUTO)
      return 0;

   /* Native gles2n64 frameskip owns the complete renderer lifecycle.
    * Never stack the global skipper on top of it. */
   if (gfx_plugin == GFX_GLN64 && retro_gles2n64_frameskip_mode != 0)
      return 0;

   if (retro_frameskip_decision_vi == retro_frameskip_vi_serial)
      return retro_frameskip_skip_current_vi;

   retro_frameskip_decision_vi = retro_frameskip_vi_serial;
   retro_frameskip_skip_current_vi = 0;

   if (retro_frameskip_auto_skip_pending)
   {
      if (retro_frameskip_auto_consecutive < RETRO_FRAMESKIP_AUTO_MAX)
      {
         retro_frameskip_auto_skip_pending = 0;
         retro_frameskip_auto_consecutive++;
         retro_frameskip_skip_current_vi = 1;
      }
      else
      {
         /* Force one complete rendered graphics frame after the limit.
...
static void retro_frameskip_update(void)
{
   if (frame_hidden || rsp_plugin != RSP_HLE ||
       retro_frameskip_mode != RETRO_FRAMESKIP_AUTO)
      return;

   /* Native gles2n64 owns skip timing when selected. */
   if (gfx_plugin == GFX_GLN64 && retro_gles2n64_frameskip_mode != 0)
      return;
   ...
   {
      const uint64_t now = (uint64_t)perf_cb.get_time_usec();
      const unsigned target_fps =
         (retro_get_region() == RETRO_REGION_PAL) ? 50u : 60u;
      uint64_t elapsed;
      uint64_t real_count;

      if (retro_frameskip_initial_usec == 0 ||
          now <= retro_frameskip_initial_usec)
      {
         retro_frameskip_initial_usec = now;
         retro_frameskip_virtual_count = 0;
         retro_frameskip_auto_skip_pending = 0;
         retro_frameskip_auto_consecutive = 0;
         return;
      ...
      elapsed = now - retro_frameskip_initial_usec;
      real_count = (elapsed * target_fps) / 1000000u;

      retro_frameskip_virtual_count++;

      if (real_count > retro_frameskip_virtual_count)
      {
         retro_frameskip_auto_skip_pending = 1;
      }
      else if (real_count < retro_frameskip_virtual_count)
      {
         retro_frameskip_virtual_count = real_count;
      }
   }
}
...
void update_variables(bool startup)
{
   ...
   /* Global renderer-independent HLE frameskip: Auto or Disabled only. */
   var.key = CORE_NAME "-frameskip";
   var.value = NULL;
   if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value)
   {
      retro_frameskip_set_mode(
         !strcmp(var.value, "auto") ? RETRO_FRAMESKIP_AUTO : 0);
   }

   /* Native Mupen64Plus-FZ-style skipper for the legacy gles2n64 renderer.
    * This is intentionally independent from the global layer. When enabled
    * while gles2n64 is active, native mode automatically takes precedence. */
   var.key = CORE_NAME "-gles2n64-frameskip";
   var.value = NULL;
   if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value)
   {
      int mode = 0;
...
void retro_run (void)
{
   ...
   /* FZ updates the skipper after the current frame. Doing the same here
    * makes the newly computed skip decision apply to the next retro_run(). */
   retro_frameskip_update();
}
