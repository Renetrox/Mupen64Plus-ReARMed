...
/* Was a verbatim zlib crc32 clone - local 256-entry table, seed and result
 * complemented at the boundaries.  encoding_crc32() is that same
 * CRC-32/ISO-HDLC with the same conventions, so this forwards directly and
 * returns identical values for every seed and length. */
