...
static void rdp_setcolorimage(uint32_t w0, uint32_t w1)
{
   ...
   if (settings.swapmode > 0)
   {
      ...
      /* Temporary Nightmare Creatures diagnostic. Keep the trace bounded so
       * it can be collected from RetroArch without flooding its log forever. */
      {
         ...
         if (log_cb && swap_diag_count < 600)
         {
            log_cb(RETRO_LOG_WARN,
                  ...
                  *gfx_info.VI_ORIGIN_REG, g_gdp.zb_address, rdp.ocimg,
                  gDP.colorImage.address, SwapOK, viSwapOK, rdp.updatescreen,
                  (g_gdp.zb_address != gDP.colorImage.address) &&
                  (rdp.ocimg != gDP.colorImage.address) && SwapOK && viSwapOK);
         }
      }
