...
/* Decide on actual HLE graphics display lists, not on VI/retro_run cadence. */
extern int retro_frameskip_should_skip_dlist(void);
...
static void send_dlist_to_gfx_plugin(struct hle_t* hle)
{
    ...
    *hle->sp_status |= SP_STATUS_TASKDONE | SP_STATUS_BROKE | SP_STATUS_HALT;
    ...
    if (retro_frameskip_should_skip_dlist())
    {
        /* Mupen64Plus FZ skips the renderer but still completes the graphics
         * task. Set the DP interrupt bit here, but do NOT call
         * HleCheckInterrupts for it: current parallel-n64 deliberately
         * consumes this bit at RSP task end and schedules DP_INT with the
         * same deferred timing used by a normally rendered display list. */
        *hle->mi_intr |= MI_INTR_DP;
    }
    else
    {
